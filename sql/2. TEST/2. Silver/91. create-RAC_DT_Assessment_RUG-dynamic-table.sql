...
CREATE OR REPLACE DYNAMIC TABLE SILVER_DB.ECASE_RAC_DT_ASSESSMENT_RUG_CONFORMED
...
assess_data AS (
    SELECT
        ...
        CASE 
            WHEN ElementName = 'RugItem_FinalScore'
            THEN SelectionValue
        END AS RugItem_FinalScore

    FROM LatestCarePlanElement L
    LEFT JOIN assesment_data A
        ON L.AssessmentFormID = A.FormID
    WHERE L.Name = 'Rug Assessment'
