...
CREATE OR REPLACE DYNAMIC TABLE SILVER_DB.ECASE_RAC_DT_ASSESSMENT_BRADEN_CONFORMED
...
assess_data AS (
    SELECT
        ...
        CASE 
            WHEN ElementName = 'BSFPPSR_Score' THEN SelectionValue
        END AS BRADEN_FinalScoreValue

    FROM LatestCarePlanElement L
    LEFT JOIN assesment_data A
        ON L.AssessmentFormID = A.FormID
    WHERE L.Name = 'Braden Scale for Predicting Pressure Sore Risk'
