...
CREATE OR REPLACE DYNAMIC TABLE SILVER_DB.ECASE_RAC_DT_ASSESSMENT_BRUA_CONFORMED
...
assess_data AS (
    SELECT
        ...
        CASE 
            WHEN ElementName = 'BRUA_FinalScoreValue' 
            THEN SelectionValue
        END AS BRUA_FinalScoreValue

    FROM LatestCarePlanElement L
    LEFT JOIN assesment_data A
        ON L.AssessmentFormID = A.FormID
    WHERE L.Name = 'Behaviour Resource Utilisation Assessment (BRUA)'
